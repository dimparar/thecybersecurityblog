+++
title = 'CTF@AC 2025: Money'
date = 2025-09-16T00:15:37+03:00
draft = false
showpage = true
+++
...
The provided code is a server that holds a widget service.

![alt](lab1.png)

The code gives away some crucial information on how the application works and what we could use to get the information out of the FLAG widget which has been uploaded automatically upon the first load of the server.

```python
# The used for the decryption is hardcoded
KEY = b"SECRET_KEY!123456XXXXXXXXXXXXXXX"
```

The uploaded file must follow the below criteria to be accepted:
- The uploaded file must end with `.plugin`
- The server must be able to decrypt it with `decrypt_file()` function
- The decrypted file then is unzipped so it must be a `.zip` file.
- The `zip` file must contain
    - `plugin_manifest.json` with the keys: `name`, `version`, `author`, `icon`
    - `init.py`

`init.py` is executed right after the plugin has been uploaded to the server and without any limitation. That means that we run arbitrary code to the server and fetch the flag through the `app.log` file.
...
def upload():
    ...
    try:
        ...
        log(f"extract_error uid={uid} err={e}")
        return jsonify({"error":"bad zip"}), 400
    manifest_path = os.path.join(plugin_dir, "plugin_manifest.json")_dir, "init.py")
    manifest = {}
    if os.path.exists(manifest_path):
        with open(manifest_path, "r") as mf:
            manifest = json.load(mf)
...
We can create a script that will masquerade this exploit.
...
with open("index.html", "w") as htmlf:
...
def encrypt_file(data, output_path, key):
    """
    Encrypts a file using AES CBC with IV as the first 16 bytes of the plaintext.
    """
    iv = data[:16]
    cipher = AES.new(key, AES.MODE_CBC, iv)
...
if __name__ == '__main__':
    ...
    encrypted_file_path = "test.plugin"
    decrypted_file_path = "test_decrypted.zip"
    ...
    # Encrypt using the hardcoded value
    encrypt_file(data, encrypted_file_path, key)
    ...
    # Verify
    with open(decrypted_file_path, "rb") as f_dec:
        assert data == f_dec.read(), "Decrypted file does not match the original!"
        print("Verification successful: Files match.")
```

Following the aforementioned rules of the application, we can build the files, zip them and feed them to the script, which will provide us with the `test.plugin` file.

If we upload the file to the application, a new widget is created. By accessing it's `index.html` we get the `app.log` file contents.
